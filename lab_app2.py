...
def home():
    ...
    if not year and not month and not dayFrom and not dayTo:
        year=datetime.today().strftime('%Y')
        month=datetime.today().strftime('%m')
        dayFrom=datetime.today().strftime('%d')
        dayTo=27
    ...
    curs.execute("SELECT Year,Month,Day,temp FROM tempTable WHERE Year=? And Month=? AND Day BETWEEN ? AND ?",(year,month,dayFrom,dayTo))
    temperatures = curs.fetchall()
    curs.execute("SELECT Year,Month,Day,humidity FROM humidityTable WHERE Year=? And Month=? AND Day BETWEEN ? AND ?",(year,month,dayFrom,dayTo))
